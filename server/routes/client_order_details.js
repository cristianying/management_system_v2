...
router.get("/:id",authorization, async (req, res) => {
    try {
      ...
      const order_products = await db.query("select * from client_order_details where order_id=$1 and user_id =$2;",
...
router.post("/:id/addProduct",authorization, async (req, res) => {
  ...
  try {
    const newProduct = await db.query(
      ...
      [req.user.id,req.params.id, req.body.product_id, req.body.box_quantity, req.body.piece_sell_price]
    );
